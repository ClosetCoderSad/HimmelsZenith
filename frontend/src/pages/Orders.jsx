    ...
    <div className="border-t pt-16 px-4 sm:px-[5vw] md:px-[7vw] lg:px-[9vw]">
      ...
      <div>
        {orderData.map((item, index) => (
          <div
            key={index}
            className="py-4 border-t border-b text-text_2 flex flex-col md:flex-row md:items-center md:justify-between gap-4"
          >
            <div className="flex items-start gap-6 text-sm ">
              <img className="w-16 sm:w-20" src={item.image[0]} alt="" />
              <div>
                <p className="sm:text-base font-medium">{item.name}</p>
                <div className="flex items-center gap-3 mt-2 text-base text-text_2">
                  <p>
                    {currency}
                    {item.price}
                  </p>
                ...
                <p className="mt-2">
                  Payment: <span className="text-policytext">{item.paymentMethod}</span>
                </p>
              </div>
            </div>
